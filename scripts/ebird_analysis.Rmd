...
head(model)
length(model$treatment)
type1 <- rep(c("model"),times= length(model$treatment))
model$type <- type1
model

#attack totals on models and mimics
attacks_species<- aggregate(x=model$attacks, by = list(model$species, model$treatment), FUN=sum)
attacks_species_day <- aggregate(x=model$attacks, by = list(model$species, model$treatment, model$experiment.day), FUN=sum)
...
#remove julian day column so data is compatible with simpson_unb function
bird_diversity_data
...
summary(sppdiv_aov) 
#no significant differences in species richness or diversity between the four treatments 
```
