...
lat <- subset(map, select = c(site, latitude) )
colnames(lat) <- c("field.site", "latitude")
head(lat)
battus_lat <- merge(battus, lat, by="field.site", all.x = TRUE)

fit_nbinom2 <- update(mod_battus2, correlation = corExp(form = ~Lon_x + Lat_y, nugget=T))
