...
head(lat_long)
battus_lat_long <- merge(battus, lat_long, by="field.site", all.x = TRUE)

mod_battus2 <- glmmTMB(attacks ~ experiment.day + transect, 
                data = battus_lat_long, ziformula=~1, family=poisson) 
summary(mod_battus2)
check_singularity(mod_battus2) #no singularity issues 
Anova(mod_battus2, type = "III") # significant effect of experiment day 
# no effect of transect (space & time) 

battus_lat_long$pos <- numFactor(battus_lat_long$longitude, battus_lat_long$latitude)
battus_lat_long$group <- factor(rep(1, nrow(battus_lat_long)))

fit_spatial_cor <- update(mod_battus2, exp(pos + 0 | group))

## trying to control for spatial autocorrelation in the model 
exponential.autocor <- update(mod_battus2,family=nbinom2)
  
  gls( log(Bird_diversity + 1) ~ log(Tree_diversity + 1) , correlation = corExp(form = ~Lon_x + Lat_y, nugget=T), data = bird.diversity )
