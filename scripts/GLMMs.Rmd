...
run to models to assess two questions: 
...
mod_battus <- glmmTMB(attacks ~ experiment.day + transect + (1|field.site/field.day), #if we include both site and field day in the model, they have to be coded this way because of the experimental design 
...
mod_battus2 <- glmmTMB(attacks ~ experiment.day + transect, 
                data = battus, ziformula=~1, family=poisson) 
summary(mod_battus2)
check_singularity(mod_battus2) #no singularity issues 
...
mod_battus3 <- glmmTMB(attacks ~ experiment.day + transect + (1|field.site), 
                data = battus, ziformula=~1, family=poisson) 
summary(mod_battus3)
check_singularity(mod_battus3) #still have singularity issues 
...
mod_battus4 <- glmmTMB(attacks ~ experiment.day + transect + (1|field.day), 
                data = battus, ziformula=~1, family=poisson) 
summary(mod_battus4)
check_singularity(mod_battus4) #still have singularity issues 
Anova(mod_battus4, type = "III")

#compare model fit with likelihood ratio test
anova(mod_battus2, mod_battus3, mod_battus4) # models are not significantly different - let's go with the one that does not include random effects beecause its the only one that didn't have singularity issues 
...
check_singularity(mod_junonia2) #no singularity issues 
...
mod1simulation <- simulateResiduals(fittedModel = mod_battus, plot = T)
...
packageVersion("DHARMa")
mod2simulation <- simulateResiduals(fittedModel = mod_junonia, plot = T)
...
# check data structure 
str(mimic)
mimic$treatment <- factor(mimic$treatment, order = TRUE, levels = c("zero", "one", "two", "four"))
...
anova(mod_T4, mod_T4_2, mod_T4_4) # no significant difference between models. Model without random effects has the lowest AIC score 
```
...
anova(mod_T2_2, mod_T2_3, mod_T2_4)
...
anova(mod_T1, mod_T1_2) #no significant difference between models 1 and 2, but we have to either keep both random effects or remove both 
```
...
```{r}
# correcting for deviaiton from uniformity by removing all sites that experienced zero attacks in testing phase 
T4_attackbysite <- aggregate(x=T4$attacks, by = list(T4$field.site), FUN=sum)
colnames(T4_attackbysite) <- c("site", "attacks")
T4_attackbysite_noNA <- subset(T4_attackbysite, attacks > 0) 
attacked_sites4 <- T4_attackbysite_noNA$site

T4transformed <- subset(T4, field.site %in% attacked_sites4)

mod_T42 <- glmmTMB(attacks ~ species + (1|field.site) + (1|field.day),
                data=T4transformed, ziformula=~1, family=poisson) 
summary(mod_T42)
check_singularity(mod_T42) 
Anova(mod_T42, type = "III") 

mod6simulation <- simulateResiduals(fittedModel = mod_T42, plot = T)
testUniformity(mod6simulation) # significant deviations from uniformity 
testOutliers(mod6simulation)
testDispersion(mod6simulation) 
testZeroInflation(mod6simulation) 
...
```{r}
# correcting for deviaiton from uniformity by removing all sites that experienced zero attacks in testing phase 
T1_attackbysite <- aggregate(x=T1$attacks, by = list(T1$field.site), FUN=sum)
colnames(T1_attackbysite) <- c("site", "attacks")
T1_attackbysite_noNA <- subset(T1_attackbysite, attacks > 0) 
attacked_sites1 <- T1_attackbysite_noNA$site

T1transformed <- subset(T1, field.site %in% attacked_sites1)

mod_T12 <- glmmTMB(attacks ~ species + (1|field.site) + (1|field.day),
                data=T1transformed, ziformula=~1, family=poisson) 
summary(mod_T12)
check_singularity(mod_T12) 
Anova(mod_T12, type = "III") 

mod6simulation <- simulateResiduals(fittedModel = mod_T12, plot = T)
testUniformity(mod6simulation) # significant deviations from uniformity 
testOutliers(mod6simulation)
testDispersion(mod6simulation) 
testZeroInflation(mod6simulation) 
...
```{r}
# correcting for deviaiton from uniformity by removing all sites that experienced zero attacks in testing phase 
T0_attackbysite <- aggregate(x=T0$attacks, by = list(T0$field.site), FUN=sum)
colnames(T0_attackbysite) <- c("site", "attacks")
T0_attackbysite_noNA <- subset(T0_attackbysite, attacks > 0) 
attacked_sites <- T0_attackbysite_noNA$site

T0_transformed <- subset(T0, field.site %in% attacked_sites)

mod_T02 <- glmmTMB(attacks ~ species + (1|field.site) + (1|field.day),
                data=T0_transformed, ziformula=~1, family=poisson) 
summary(mod_T02)
check_singularity(mod_T02) 
Anova(mod_T02, type = "III") 

mod6simulation <- simulateResiduals(fittedModel = mod_T02, plot = T)
testUniformity(mod6simulation) # significant deviations from uniformity 
testOutliers(mod6simulation)
testDispersion(mod6simulation) 
testZeroInflation(mod6simulation) 
...
b.plot <- ggplot(battus_plot_data) + 
  ...
  geom_line(data = p_value4, 
            aes(x = x, y = y, group = 1)) + 
  geom_text(x=1.6, y=23.75, label="P=0.0131*") +
  geom_text(x=2, y=25.25, label="P=0.0131*") +
  geom_text(x=2.5, y=26.75, label="P=0.0114*") 
