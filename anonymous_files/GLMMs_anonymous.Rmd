...
```{r pull model data}
m <- curl(".../data/Quabbin_Model_Data.csv")
...
d <- curl(".../data/Quabbin_GIS_Data.csv")
...
## goodness of fit comparison with model without spatial correction 

## mod2 without spatial correction 
mod2_2 <- glmmTMB(attacks ~ experiment.day*species + transect, 
                data = model, ziformula=~1, family=poisson) 
summary(mod2_2)
check_singularity(mod2_2) #no singularity issues
Anova(mod2_2, type = "III") 

## mod4 without spatial correction 
mod4_2 <- glmmTMB(attacks ~ experiment.day*species + transect + (1|field.day),
                data = model, ziformula=~1, family=poisson) 
summary(mod4_2)
check_singularity(mod4_2) #false 
Anova(mod4_2, type = "III") 

## model comparisons
anova(mod2, mod4, mod2_2, mod4_2) 
```
...
```{r pull mimic data }
c <- curl(".../data/predation_data/Quabbin_Mimic_Data.csv")
...
mimic$group <- factor(rep(1, nrow(mimic)))

#all random effects 
mod5 <- glmmTMB(attacks ~ species*treatment + (1|field.site) + (1|field.day) + exp(pos + 0 | group), 
...
## run mod5 without spatial correlation correction 
mod5_2 <- glmmTMB(attacks ~ species*treatment + (1|field.site) + (1|field.day), 
                data = mimic, ziformula=~1, family=poisson) 
summary(mod5_2)
Anova(mod5_2, type = "III") 

## run mod8 without spatial correlation correction 
mod8_2 <- glmmTMB(attacks ~ species*treatment + (1|field.day), 
                data = mimic, ziformula=~1, family=poisson) 
summary(mod8_2)
Anova(mod8_2, type = "III") 

#run model comparisons 
anova(mod5, mod8, mod5_2, mod8_2) 
## mod 5 is significantly better - lower AIC score 
...
EMM3 <- emmeans(mod5_2,  pairwise ~ species|treatment)
pairs(EMM3)
pairs(EMM3, adjust = "bonferroni")
```
...
s <- curl(".../data/predation_data/synch_treatment_data.csv")
...
quab.map <- ggmap(get_googlemap(center = c(lon = -72.30, lat = 42.398),
            ...
            theme(legend.position="right", 
             ...
             scale_shape_manual(breaks = c("one", "two", "four", "sim"), 
                                ...
                                labels = c("Transect 1", "Transect 2", "Transect 3", "Transect 4"))
quab.map
ggsave(quab.map, file = ".../quab.map.pdf", width=15, height=10, units=c("in"), useDingbats=FALSE)
...
ggsave(attack.fig, file = ".../attack_fig.pdf", width=8, height=8, units=c("in"), useDingbats=FALSE)
